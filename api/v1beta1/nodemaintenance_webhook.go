...
const (
	...
	ErrorControlPlaneQuorumViolation = "can not put control-plane node into maintenance at this moment, it would violate the control-plane node quorum"
)
...
func (v *NodeMaintenanceValidator) validateControlPlaneQuorum(nodeName string) error {
	...
	if node, err := getNode(nodeName, v.client); err != nil {
		return fmt.Errorf("could not get node for control-plane quorum validation, please try again: %v", err)
	} else if node == nil {
	...
	for _, pdbName := range []string{EtcdQuorumPDBNewName, EtcdQuorumPDBOldName} {
		...
		if err := v.client.Get(context.TODO(), key, &pdb); err != nil {
			...
			return fmt.Errorf("could not get the etcd quorum guard PDB for control-plane quorum validation, please try again: %v", err)
		}
	...
	if disruptionsAllowed == -1 {
		...
		nodemaintenancelog.Info("etcd quorum guard PDB hasn't been found. Skipping control-plane quorum validation.")
		return nil
	}
