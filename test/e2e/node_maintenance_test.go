...
var _ = Describe("Node Maintenance", func() {
	...
	Describe("Starting maintenance", func() {
		...
		Context("for the 1st control-plane node", func() {
			...
			It("should succeed", func() {
				if len(controlPlaneNodes) < 3 {
					Skip("cluster has less than 3 control-plane nodes and is to small for running this test")
				}
			...
			It("should fail", func() {
				if len(controlPlaneNodes) >= 3 {
					Skip("with 3 or more control-plane it should not fail")
				}
		...
		Context("for the 2nd control-plane node", func() {
			...
			AfterEach(func() {
				...
				if controPlaneMaintenance != nil {
					if err := Client.Delete(context.TODO(), controPlaneMaintenance); err != nil {
						logWarnf("failed to delete NodeMaintenance for 1st control-plane node: %v\n", err)
					}
			...
			It("should fail", func() {
				if len(controlPlaneNodes) < 3 {
					Skip("cluster has less than 3 control-plane nodes and is too small for running this test")
				}
				if len(controlPlaneNodes) > 3 {
					logWarnf("there are %v control-plane nodes, which is unexpected. Skipping quorum validation for 2nd control-plane node!\n", len(controlPlaneNodes))
					Skip("unexpected big cluster, no clue if 2nd control-plane maintenance is fine or not")
				}
...
func getNodes() ([]string, []string) {
	...
	logInfof("control-plane nodes: %v\n", controlPlaneNodes)
